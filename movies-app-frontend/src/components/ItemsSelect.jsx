...
const ItemsSelect = ({ items, itemsSelected, setItemsSelected, isMultiple=true, itemStructure }) => {
  ...
  return (
    <div className="items-select">
      {/* Selector de items */}
      <Form.Select
        className="mb-3 form-control-soft"
        onChange={e => e.target.value && selectItem(+e.target.value)}
      >
            ...
            return (
              <div className="form-card-container" key={itemId}>
                <button
                  ...
                  onClick={() => deselectItem(itemId)}
                >
                  <i className="fa-solid fa-trash"></i>
                </button>
                <Card className="form-card">{structure(item)}</Card>
              </div>
            );
          })
        ) : (
