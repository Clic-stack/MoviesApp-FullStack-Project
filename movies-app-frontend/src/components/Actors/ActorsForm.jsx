...
import { addActorThunk, updateActorThunk } from '../../store/slices/actors.slice';
...
const ActorsForm = ({ show, handleClose, actorSelected }) => {
    ...
    useEffect(() => {
        if (actorSelected) {
            const normalized = {
                firstName: actorSelected.first_name,
                lastName: actorSelected.last_name,
                birthday: actorSelected.birthday,
                nationality: actorSelected.nationality,
                image: actorSelected.image
            };
            setActor(normalized);
        } else {
    ...
    const saveActor = () => {
        ...
        if (!actorSelected) {
            dispatch(addActorThunk(normalizedActor));
        } else {
            dispatch(updateActorThunk(actorSelected.id, normalizedActor));
        }
